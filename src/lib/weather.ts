...
export async function getDenverWeather(): Promise<WeatherData | null> {
  ...
  if (cached) {
    ...
    if (age < TTL_MS) return cached as WeatherData;
  }

  if (!API_KEY) return cached as WeatherData | null;

  try {
    const res = await fetch(
      `https://api.weatherapi.com/v1/current.json?key=${API_KEY}&q=${encodeURIComponent(CITY)}&aqi=no`
    );
    if (!res.ok) return cached as WeatherData | null;
    ...
    await supabase
      .from("weather_cache")
      .upsert(row, { onConflict: "city" });

    return row as WeatherData;
  } catch {
