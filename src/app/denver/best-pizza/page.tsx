...
function PizzaCard({ place, rank }: { place: Place; rank?: number }) {
...
export default async function BestPizzaPage() {
  ...
  const places = raw.filter(isRealRestaurant).filter((p) => p.types?.includes("pizza_restaurant"));
    ...
    <>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        ...
        <div className="mt-4 text-sm text-slate-500">
          <strong className="text-foreground font-bold">{places.length}</strong> pizzerias tracked citywide
        </div>
      </section>
      ...
      {top.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {top.map((place, i) => <PizzaCard key={place.place_id} place={place} rank={i + 1} />)}
          </div>
        </section>
      )}
