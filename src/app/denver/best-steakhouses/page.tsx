...
function SteakhouseCard({ place, rank }: { place: Place; rank?: number }) {
...
export default async function BestSteakhousesPage() {
  ...
  const places = raw.filter(isRealRestaurant).filter((p) => p.types?.includes("steak_house"));
      ...
      {top.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {top.map((place, i) => <SteakhouseCard key={place.place_id} place={place} rank={i + 1} />)}
          </div>
        </section>
      )}
